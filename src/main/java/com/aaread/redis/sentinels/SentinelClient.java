...
import com.aaread.redis.sentinels.SentinelProtocol.Command;
...
public class SentinelClient extends SentinelConnection {
	...
	public SentinelClient(final String host) {
		super(host);
	}
	...
	public void getMasterIp(String masterName){
		sendCommand(Command.SENTINEL, SafeEncoder.encode("get-master-addr-by-name"),SafeEncoder.encode(masterName));
	}
    ...
    public void sentinelReset(String pattern) {
    	sendCommand(Command.SENTINEL, SafeEncoder.encode("reset"),SafeEncoder.encode(pattern));
    }
}
